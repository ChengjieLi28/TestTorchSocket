...
class Store {
 public:
  ...
  virtual std::vector<uint8_t> compareSet(
      const std::string &key, const std::vector<uint8_t> &currentValue,
  ...
  virtual void watchKey(const std::string & /* unused */,
    ...
    //    TORCH_CHECK(
    //        false,
    //        "watchKey only implemented for TCPStore and PrefixStore that wraps
    //        TCPStore.");
    throw std::runtime_error(
        "watchKey only implemented for TCPStore and "
